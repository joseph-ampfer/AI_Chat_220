...
// Constants
const JSON_BLOB_URL = 'https://jsonblob.com/api/jsonBlob/1344685475717373952';
const GROQ_ENDPOINT = 'https://api.groq.com/openai/v1/chat/completions';
const PUBLIC_POSTS_URL = 'https://jsonblob.com/api/jsonBlob/1346491622271148032';
  ...
  sections.forEach(section => {
    if (groupedChats[section].length > 0) {
      ...
      for (let i = groupedChats[section].length - 1; i >= 0; i--) {
        ...
        renameChatBtn.classList.add('rename-chat', 'btn', 'd-flex', 'justify-content-between', 'text-end');
...
async function sendMessageToAI(message) {
  try {
    ...
    } else if (currentChatIndex == -2) { // Loaded message from index page
